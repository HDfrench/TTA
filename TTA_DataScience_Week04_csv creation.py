        ...
        if data["layout"] != 8:
            for header, details in data["placeholders"].items():
                ...
                for plh, cnt in details.items():
                    p = self.slide.shapes[int(header)].text_frame.add_paragraph()
                    p.text = cnt["text"]
                    p.level = cnt["level"]
                    p.font.size = Pt(int(cnt["size"]))
                    p.font.bold = cnt["bold"]
                    p.font.italic = cnt["ital"]
                    if cnt["align"] == "left":
...
def main():
    ...
    def scatter_plot_image(dataset, colx, coly, name, display):
        myplot = dataset.plot(kind='scatter', figsize=(6, 4), title=display, x=colx, y=coly)
    ...
    scatter_plot_image(dataset, 'Feedback', 'All-inc', 'Comp_feedback_allInc.jpg', 'Comparison Feedback / All-inclusive')
    ...
    slide_content = {1: {"layout":0, "title": "Using Pandas",
        ...
        7: {"layout": 2, "title": 'Some insights 5/5',
            "placeholders":{1:
                ...
                3: {"level": 2,
                    "align": "left",
                    "text": "The lower it is, the higher tourit interest",
                    "size": 10,
                    "bold": False,
                    "ital": True},
                4: {"level": 1,
                ...
                6: {"level": 1,
                    "align": "left",
                    "text": "The ratio ot all-inclusive accommodations compared to all rental accommodation",
                    "size": 12,
                    "bold": False,
                    "ital": False},
                7: {"level": 2,
                ...
                8: {"level": 1,
                    "align": "left",
                    "text": "The ratio ot the average number of stars per accommodations",
                    "size": 12,
                    "bold": False,
                    "ital": False},
                9: {"level": 2,
